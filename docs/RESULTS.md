...
| D-78 | *(réservé — l'artefact T29 (`t29_loso_delta_ci_N256_perscenario.npz`) ne se reproduit plus à HEAD : un Δ change de signe, le verdict `dim=4` passe de « nuisent » à « indécidable ». Rapport seul)* | *en cours* | *en cours* |
| D-79 | *(réservé — `verdict()` de T29 écarte un fold parce qu'un prédicteur qui n'entre pas dans la quantité comparée s'est effondré sur une constante)* | *en cours* | *en cours* |
