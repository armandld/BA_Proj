...
| D-152 | **Le garde ecrit apres la fuite d'identifiant Neon (D-65) ne balaie que `src/*.py` : la fuite suivante n'a qu'a se poser ailleurs — et « ailleurs » est justement ou vit une chaine de connexion.** `test_no_hardcoded_credential_url_in_src` (`tests/pipeline/test_no_credential_in_source.py`) parcourait 25 fichiers de `src/` ; le depot en porte **409** de type texte, dont **384 hors `src/`** — les **10 lanceurs `.sh`** (dont ceux qui exportent `OPTUNA_STORAGE`), tout `study/`, tout `scripts/`, `environment.yaml`, les `.json`. Le depot est **public** et l'historique porte deja un mot de passe non tourne : le garde est la seule chose qui empeche le suivant. **Second trou, meme famille que D-151** : la lecture etait ligne a ligne, donc une URL coupee par une continuation `\` en fin de ligne — la forme normale d'un long `export` shell — n'etait vue par personne. Corrige : balayage de tout le depot par extension (les binaires et `Train_results/`, `Data_results/` exclus), lignes logiques recollees avant lecture, et les modeles de documentation (`user:pass`, `user:pw`, …) exemptes **par couple utilisateur:motdepasse**, nommement, avec leur raison — plus un test qui exige que chaque modele existe encore, pour qu'une exemption perimee crie | **A'** — un `export OPTUNA_STORAGE="postgresql://…@ep-cool.aws.neon.tech/db"` ajoute a `scripts/run_fold.sh` : ancien garde **2 passed**, nouveau **1 failed, 7 passed** (`scripts/run_fold.sh:40`). **A''** — le meme secret coupe par une continuation `\` : aucune ligne physique ne repond au motif (champ qui SEPARE, verifie dans le test), le garde le voit sur la ligne logique. Arbre sain : ancien 2 → nouveau **8 passed**. **Rien de reel n'a ete trouve** : les 4 URL a mot de passe du depot sont des modeles de documentation (`README.md`, `docs/MODE_EMPLOI_CAMPAGNE.md`, ce fichier ×2). **D-65 reste ouvert** — la rotation du mot de passe publie est une action de USER, aucun test ne peut la faire | `pytest tests/pipeline/test_no_credential_in_source.py -q` |
