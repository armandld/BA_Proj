...
| D-133 | **RÉSERVÉ — le garde anti-D15 (« plus aucun appel à `git_commit_hash()` dans les tâches longues ») est défait par un alias d'import.** Mutation A′ (D15 REINTRODUIT — `git_hash` réécrit après `provenance.finish` via `from provenance import git_commit_hash as _gch`) : `grep -c "git_commit_hash()"` → **0**, et le fichier reste **7 passed**. Mesure faite, ligne complétée par le commit suivant | — | — |
