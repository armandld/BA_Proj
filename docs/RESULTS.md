...
| D-131 | **RÉSERVÉ — la « définition unique » du seuil classique de la campagne en boucle fermée n'est gardée que par le SOURCE.** Mutation A′ (le seuil redevient une constante RECOPIÉE, la chaîne cherchée laissée en code mort sous `if False:`) → **12 passed**, faux vert — exactement la divergence que le commentaire du test annonce empêcher ; mutation B (`FROZEN_DEFAULTS` réécrit en littéral ÉQUIVALENT) → **ROUGE**. Mesure faite, ligne complétée par le commit suivant | — | — |
