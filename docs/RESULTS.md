...
| D-113 | **RÉSERVÉ** — `create_bounded_hamiltonian` : la contraction de plaquette du bord remplace le qubit manquant par le `<Z>` de l'**autre famille de liens** (le lien Droite est un lien V et reçoit un `<Z>` calculé sur `theta_h_full` ; le lien Bas est un lien H et reçoit un `<Z>` calculé sur `theta_v_full`). Mesure en cours | — | — |
