...
| D-97 | **Réservé** — `fig9_synthetic_unit_tests.py` : les trois générateurs de champ physiquement motivés (`make_vortex_core`, `make_current_sheet`, `make_xpoint`) échangent X et Y dans le dépaquetage de `np.mgrid`/la diffusion du broadcast, contre la convention du dépôt (`AXIS_X=0`, `AXIS_Y=1`, `indexing='ij'`) — `∇·B ≠ 0` par construction, jamais nettoyé (`PROJECT_B=False`) | mesure en cours | `pytest tests/study/test_fig9_synthetic_fields_solenoidal.py` |
