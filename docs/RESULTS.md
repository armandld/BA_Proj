...
| D-178 | **Le résumé cross-Re de la phase 4 relisait un autre artefact que celui qu'elle venait d'écrire.** `save_results` nomme l'artefact `exact_diag_..._dim{D}{sfx}.npz` (`sfx = "_v2"` sous `--v2`) ; le résumé cross-Re de `main()` (`study/pipeline/exact_diagonalisation.py`) reconstruisait ce nom à la main, **sans le suffixe**. `study/common/qaoa_inputs.py:689` (phase 5), l'autre lecteur de ces artefacts, applique bien le suffixe : c'est le **producteur qui divergeait de son propre consommateur** (question 4 de `VIGIL.md`). Trouvé en reprenant la piste que le commentaire du 20 août nommait — `study/pipeline/` lu en entier mais aucun test n'en traversant les axes. **Mesuré, avant, à `c7292a1`** sur `harris_tearing Re=400 N=256 dim=2`, `RESULTS_DIR` isolé : (a) relance fraîche `--v2` seul → l'en-tête « PHASE 4 CROSS-Re SUMMARY » suivi de **0 ligne**, code de sortie **0** ; (b) artefact v1 présent puis `--v2` → le résumé imprime **la ligne du v1**. (a) est l'ordre exact de `scripts/run_study_v2b.sh` phase 4, qui lance `--v2` d'abord — c'est-à-dire la relance que `BRIEF_REPRISE.md` §8 point 2 demande, et c'est la famille D-55/D-56/D-75/D-148 **dans la fonction même** qui lève déjà pour un balayage vide douze lignes plus haut. (b) ne rend **pas** de nombre faux aujourd'hui : D-47 rend les deux prédicteurs constants, donc le F1 ne dépend que de la vérité terrain et sort identique en v1 et en v2 sur les quatre scénarios canoniques (**0,3333 / 0,3333 / 0,3333 / 0,3733**). Les deux artefacts sont pourtant des objets distincts — E0 moyen **−2039,70** (v1) contre **−11,15** (v2) sur `orszag_tang`, **−0,0503** contre **−12,4242** sur `harris_tearing` : le jour où D-47 est levé, le F1 cesse d'être insensible au hamiltonien et la ligne devient fausse. Hors chemin critique au sens de la règle d'arrêt (aucun nombre publié, ne bloque pas la réoptimisation) : une ligne ici, **pas d'entrée `DEFAUTS.md`** | Un seul constructeur de nom, `artifact_name()`, écrit ET relu — la correction retire le second site plutôt que de le réparer. **Mesuré, après**, mêmes conditions : (a) **1 ligne** de résumé, l'artefact `_v2` ; (b) **1 ligne**, celle du `_v2`. Trois tests, qui mordent : sur l'arbre d'avant **3 failed** — (a) « 0 ligne(s) pour 1 attendue », (b) « le résumé --v2 a imprimé l'artefact v1 : `exact_F1=0.999` » sur un artefact v1 **planté** à 0,999 (mutation de l'artefact, pas du test), (c) `artifact_name` absent — sur l'arbre corrigé **3 passed**. Le test (b) épingle l'ancien comportement | `pytest tests/study/test_phase4_summary_reads_what_it_wrote.py -q` → 3 passed |
| D-179 | **`build_patch_hamiltonian` (`study/pipeline/exact_diagonalisation.py:57`) rend TROIS valeurs ; sa docstring n'en annonçait que **deux**, et donnait au second — `score: (N, N) classical score` — la forme du troisième.** Mesuré (N=32, dim=2, champ analytique) : la 2ᵉ valeur a la forme **(2, 2)**, la 3ᵉ la forme **(32, 32)**. Ce sont deux scores classiques de **même type et de même intervalle** sur des grilles différentes — la 2ᵉ est le `block_max` de la 3ᵉ — c'est-à-dire exactement la forme de piège de la question 3 de `VIGIL.md` (« deux quantités du même nom, du même type, du même intervalle »), celle qui a coûté D-9. Trouvé en épuisant le module rouvert par D-178. **Le piège est armé et n'a jamais tiré** : les **15** sites d'appel de `study/` et `tests/` déballent tous trois valeurs, et chacun prend celle qu'il veut (`score_vqa` en 2ᵉ, `full_score`/`v2_full` en 3ᵉ) — vérifié site par site. Aucun nombre publié n'en dépend. Hors chemin critique : une ligne ici, **pas d'entrée `DEFAUTS.md`** — même règle que D-175 et D-176 | Docstring réécrite pour dire les trois valeurs et la forme de chacune. **Comportement inchangé** : aucune ligne de calcul touchée. Le contrat est désormais épinglé par un test de FORMES (pas de texte de source) : `len(out) == 3`, `score_vqa.shape == (dim, dim)`, `full_score.shape == (N, N)`, et `score_vqa == block_max(full_score)` — le champ qui sépare, un score non constant rendant une réduction `max` distincte de toute autre. Il mord : mutation `return hamilt_params, full_score, score_vqa` (les deux scores échangés) → **3 failed, 1 passed** ; arbre sain → **4 passed** | `pytest tests/study/test_phase4_summary_reads_what_it_wrote.py -q -k docstring_says` |
| D-180 | **⚠️ Porte une lecture publiée — la provenance est corrigée ici, la LECTURE reste ouverte (`DEFAUTS.md` D-180).** `pipeline_verification.analyze` (phase 6), branche `--v1`, choisissait l'énergie par `for k in coefs.files: if k.endswith("_E"): break` — la **première clé du `.npz`**, pas un sigma choisi. La phase 3 en écrit **six** dans le même artefact (`sigma_values = [0.023, 0.05, 0.10, 0.15, 0.20, 0.30]`). **Le verdict F1 flippe sur le sigma seul**, à échantillon apparié — les mêmes 2 lignes sur 4 (`orszag_tang`, `mhd_rotor`) survivent au tri de D-40 à **tous** les sigmas, les deux autres sont dégénérées partout : **0,023 → PASS** (AUC 0,8737, F1 0,7288), 0,050 → PASS (0,8653 / 0,6988), 0,100 → **WARN** (0,8351 / 0,6114), 0,150 → **TIE** (0,8253 / 0,6372), 0,200 → **WARN** (0,8194 / 0,6256), 0,300 → **WARN** (0,7932 / 0,6269), contre F1(classique) **0,6543** partout. Deux exécutions de la mesure **bit-à-bit identiques**. Les nombres publiés par **D-40** (0,874 / 0,729 / 0,654 / PASS) et re-vérifiés par **D-77** sont ceux de `0,023 = TRAINED_SIGMA` — atteint parce que `sigma_values` est écrit dans cet ordre, c'est-à-dire par l'ordre des membres du `.npz` : réordonner cette liste de balayage aurait changé le verdict publié sans qu'aucun mot de la sortie ne bouge. Aucune ligne n'imprimait le sigma ; l'en-tête disait « v1 (trained) ». Trouvé par la question 3 de `VIGIL.md` en épuisant `study/pipeline/`. Formes du tableau : **repli silencieux** et **valeur sans provenance** | `--sigma` (défaut `TRAINED_SIGMA`), clé **construite** et non devinée, **`KeyError` nommant les sigmas disponibles** si le demandé manque, et le sigma imprimé dans l'en-tête et dans **chaque** ligne de verdict. **Aucun nombre publié ne bouge** : `python study/pipeline/pipeline_verification.py --v1 --dim 4 --re 400 --N 256`, `diff` avant/après → **4 lignes**, toutes des ajouts de `sigma=0.023`, **aucun chiffre modifié** (AUC 0,874, F1 0,729 vs 0,654, PASS/PASS identiques). 9 tests. Ils mordent, mesurés par **mutation de la logique de sélection à signature conservée** (le repli sur la première clé remis) : **7 failed, 2 passed** — les 2 qui passent sont exactement ceux qui ne peuvent pas séparer (`sigma = 0,023` et les nombres publiés), ce qui est la règle « choisir le champ qui SÉPARE » mesurée sur elle-même ; arbre sain **9 passed** | `pytest tests/study/test_phase6_sigma_has_a_provenance.py -q` → 9 passed |
| *(note, décision requise — rien n'est corrigé)* | **`study/pipeline/sanity_check.py:56-61` recopie les 9 hyperparamètres entraînés au lieu de les importer de `config.py`, et n'importe pas `config` du tout.** Balayage de `study/`, `src/` et `figures/` (hors `v1_legacy/`) : c'est la **seule duplication exécutable** du jeu entraîné hors de `config.py` — les autres occurrences de `0.1496` / `9.94` sont de la prose, et `h2b_psi_feature_loso.py:58` montre la convention du dépôt (`from config import TRAINED_BETA as ...`). **Mesuré : les 9 valeurs coïncident aujourd'hui, 0 désaccord** — piège armé, non déclenché. Ce qui l'arme est **programmé** : D-22 (réoptimisation, ~206 h CPU, `BRIEF_REPRISE.md` §8 point 1) va réécrire ces 9 valeurs dans `config.py`, et ce jour-là la phase 0.3 comparerait le v2 courant à un **v1 périmé** en se présentant comme le contrôle de santé du hamiltonien déployé, sans qu'un mot de sa sortie ne bouge. Trouvé par la question 4 en épuisant `study/pipeline/` | **Non corrigé, et volontairement** — c'est un défaut *contre* un choix de conception (un gel délibéré des paramètres publiés serait défendable), et `VIGIL.md` dit alors : mesurer, documenter, ne pas corriger, demander. Ce qui est ajouté est un **garde qui refuse le silence**, pas une correction : 3 tests qui comparent les 9 valeurs à `config.py` et dont le message d'échec pose la question — *suivre `config`, ou geler et l'écrire dans le fichier comme `dns_validation.py` le fait ; ne pas se contenter de recopier la nouvelle valeur, c'est ce qui a rendu la divergence invisible la première fois*. Le 3ᵉ test garde la **couverture du garde** lui-même. Ils mordent : mutation `TRAINED_KAPPA = 10.0 → 7.3` (ce que la réoptimisation fera) → **1 failed, 2 passed** ; clé ajoutée à `V1_PARAMS` sans mise à jour du garde → **1 failed, 2 passed** ; arbre sain → **3 passed** | `pytest tests/study/test_sanity_check_params_track_config.py -q` |
