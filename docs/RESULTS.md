...
| D-154 | **RÉSERVÉ — le garde qui doit voir un fichier déplacé regarde 3 des 352 imports internes de `tests/`, et 45 tests disparaissent en SILENCE.** `tests/test_suite_integrity.py` ne vérifie que les imports dont le module commence par `tests.` ; les 349 autres (`Simulation`, `VQA`, `pipeline`, `train_hyperparams`…) et les 15 `pytest.importorskip` ne sont vus par personne. Mesure et correction dans la même passe | en cours | `pytest tests/test_suite_integrity.py -q` |
