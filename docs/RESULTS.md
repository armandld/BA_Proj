...
| D-94 | **Réservé** — `figures/v1_legacy/fig0_pareto_lambda.py` : `TRAIN_DIR` et `JSON_PATH` ancrés un niveau trop haut ET sur des chemins que la réorganisation a vidés ; le script meurt à sa première lecture | mesure en cours | `pytest tests/study/test_fig0_pareto_paths.py` |
