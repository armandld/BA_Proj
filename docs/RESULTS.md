...
| D-110 | **RÉSERVÉ — `run_tests.sh`, le lanceur que `README.md` documente et que `docs/protocol_v3_evaluation.md` érige en critère d'acceptation, ne peut plus exécuter un seul de ses étages : ses 17 chemins pointent vers l'arborescence `tests/` d'avant la réorganisation.** Ligne posée avant écriture pour réserver le numéro (protocole `VIGIL.md`). Mesure, correction et test suivent dans le même fil. | en cours | en cours |
| D-111 | **RÉSERVÉ — deux lanceurs de `scripts/` calculent une racine de dépôt décalée d'un niveau, dans les deux sens : `generate_figures_v1.sh` cherche tout sous `scripts/`, `run_study_v3.sh` cherche tout dans le PARENT du dépôt.** Même cause que D-110 : les scripts ont été déplacés dans `scripts/`, leur `ROOT_DIR` n'a pas suivi. Ligne posée avant écriture. | en cours | en cours |
