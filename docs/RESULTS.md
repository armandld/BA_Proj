...
| D-150 | **La conclusion de fig15 est gardée par deux chaînes du source : elle peut citer de nouveau le σ d'un autre module sans qu'aucun test ne bouge, et une simple réécriture de guillemets la fait rougir.** `test_source_prints_the_dynamically_computed_sigma` et `test_source_no_longer_hardcodes_the_other_modules_sigma` (`tests/study/test_fig15_sigma_narration.py`), le garde de D-102. Réservé — mesures et correction poussées ensuite. | réservé | `pytest tests/study/test_fig15_sigma_narration.py -q` |
