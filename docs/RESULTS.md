...
| D-156 | **RÉSERVÉ — le garde de D-140 traite un bloc ``` comme un span de code inline : il aplatit le bloc entier, la PREMIÈRE commande avale les options de toutes les suivantes, et les suivantes ne sont jamais vues.** 14 commandes à options lues sur 21 réellement présentes dans `RESULTS.md`. Mesure et correction dans la même passe | en cours | `pytest tests/study/test_repro_commands_point_to_real_files.py -q` |
