...
| D-157 | **RÉSERVÉ — le balayage COMPORTEMENTAL de D-148 envoie `--scenario` à 21 des 61 modules qui ne le déclarent pas : ils meurent dans argparse (`exit 2`) et le test, qui n'exige qu'un code non nul, passe sans que le module ait exécuté une seule ligne.** C'est le piège du balayage vide, dans le fichier écrit pour le fermer. Mesure et correction dans la même passe | en cours | `pytest tests/study/test_empty_sweep_never_silent.py -q` |
