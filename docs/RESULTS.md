...
| D-99 | **Réservé** — `fig3_spatial_coherence.py` : `compactness` et `component_density` traitent le domaine comme BORNÉ alors que la grille est périodique ; une structure qui traverse le bord est comptée comme incohérente | mesure en cours | `pytest tests/study/test_fig3_periodic_coherence.py` |
