...
| D-95 | **Réservé** — `fig0_pareto_lambda.py` : le bras classique est tronqué à la fenêtre de score du bras quantique avant son front de Pareto et son étoile « Best Classical » ; tout essai classique meilleur que TOUT le quantique est jeté par construction | mesure en cours | `pytest tests/study/test_fig0_classical_truncation.py` |
| D-96 | **Réservé** — `fig15_decision_flip_analysis.py`, `fig16_decision_landscape.py`, `fig17_topological_attribution.py` : régression de D-37 dans leur `instrumented_bfs`/`instrumented_bfs_hamilt` local — `_process_score(local_score_raw, is_periodic, target_dim + 2*pad)` redemande le halo que la fonction ajoute déjà, décalant `classical_score` sur la mauvaise sous-région de la grille à `depth > 0` | mesure en cours | `pytest tests/study/test_v1_legacy_instrumented_bfs_score_grid.py` |
