...
| D-153 | **Le garde anti-rotationnel-prive — la famille de defauts la plus dangereuse du depot — cherche une FORME avec une liste de mots tenue a la main, et ne balaie pas `tests/`.** `test_no_new_hand_rolled_curl_uses_a_bare_axis_number` (`tests/study/test_no_private_curl_survives.py`). Reserve — mesures et correction poussees ensuite. | reserve | `pytest tests/study/test_no_private_curl_survives.py -q` |
