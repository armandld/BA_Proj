...
| D-132 | **RÉSERVÉ — le garde qui empêche le retour de D-96/D-37 est défait par DEUX ESPACES, et rougit sur le commentaire que `VIGIL.md` exige.** Mutation A′ (`target_dim+2*pad`, le défaut réintroduit sans espaces) → **7 passed** ; mutation B (un commentaire correct qui documente la déviation) → **ROUGE**. Mesure faite, ligne complétée par le commit suivant | — | — |
