...
| D-144 | **Réservé — les quatre gardes de la décision D-47 lisent le SOURCE, et deux d'entre eux ne peuvent pas rougir.** `tests/study/test_phase5_ne_filtre_plus_sur_promising.py` garde les deux moitiés de la décision de USER sur D-47 (la phase 5 ne filtre plus ; le diagnostic reste imprimé). Mutations A′ en cours de mesure | en cours | `pytest tests/study/test_phase5_ne_filtre_plus_sur_promising.py -q` |
