...
| D-159 | **RÉSERVÉ — les deux tests qui doivent garantir que « couvert » veut dire « un test le vise vraiment » NE PEUVENT PAS ÉCHOUER : l'inventaire qui déclare les noms est lui-même dans le corpus qu'ils fouillent.** Un module neuf de `src/`, déclaré COVERED et critique, qu'aucun test ne nomme : **102 passed**. Mesure et correction dans la même passe | en cours | `pytest tests/pipeline/test_src_coverage_inventory.py -q` |
