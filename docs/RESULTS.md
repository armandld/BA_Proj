...
| D-145 | **Réservé — le balayage qui interdit la fuite du JSON dans la boucle fermée ne voit qu'une des deux façons de lire `hp`.** `_live_pipeline_keys()` (`tests/study/test_hyperparams_two_sources.py`) énumère les clés par un regex sur `hp.get('…')`. En cours de mesure | en cours | `pytest tests/study/test_hyperparams_two_sources.py -q` |
