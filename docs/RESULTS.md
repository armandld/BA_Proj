...
| D-103 | **Réservé** — `tests/pipeline/test_import_never_destroys_destination.py::test_deletion_happens_only_in_this_script` balaie tout le système de fichiers (`grep -rn`), pas seulement les fichiers suivis par git : un `.venv/` local (anticipé par le `.gitignore` du dépôt) fait échouer le test sur le propre code d'`optuna` | mesure en cours | `pytest tests/pipeline/test_import_never_destroys_destination.py` |
