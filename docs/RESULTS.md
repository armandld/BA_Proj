...
| D-128 | **RÉSERVÉ — `test_aggregations_over_runs_filter_completed` sélectionne 0 ligne sur 65 scripts : le balayage vide, dans le fichier qui existe pour le détecter.** Mesuré : 0 ligne vue, 2 agrégations réelles. Ligne complétée par le commit suivant | — | — |
| D-129 | **RÉSERVÉ — le suffixe `_globalthr` de T28 : le garde qui empêche la variante d'écraser l'artefact par scénario ne lit que le SOURCE.** Mutation A′ (suffixe retiré du chemin écrit, les DEUX chaînes cherchées intactes) → **72 passed**, faux vert ; mutation B (réécriture ÉQUIVALENTE, chemin bit à bit identique) → **ROUGE**, faux rouge. Mesure faite, ligne complétée par le commit suivant | — | — |
