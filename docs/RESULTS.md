...
| D-134 | **RÉSERVÉ — `--mode leak-free` de T22 : les quatre chaînes qui gardent le débranchement de la fuite D13 laissent passer sa réintroduction.** Mutation A′ (le seuil QAOA réécrit sur `LEAKED_THRESHOLD` juste après, les QUATRE chaînes intactes) → **35 passed**, sous un artefact nommé `leak-free`. Mesure faite, ligne complétée par le commit suivant | — | — |
