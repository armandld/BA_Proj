...
| D-102 | **Réservé** — `fig15_decision_flip_analysis.py` : le bloc CONCLUSION cite en dur « σ=0.023 (trained) », qui est `TRAINED_SIGMA` de `study/pipeline/config.py`, pas le repli (0,05) que le `HamiltMapper` de CE fichier utilise réellement (`_hamilt_mapper_kwargs`, `fig_utils.py`) | mesure en cours | `pytest tests/study/test_fig15_sigma_narration.py` |
