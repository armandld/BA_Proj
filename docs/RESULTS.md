...
| D-126 | **RÉSERVÉ — `test_the_empty_sweep_message_names_what_is_missing` s'ancre sur une phrase non unique et rougit sur un changement VOULU.** Révélé par mon propre commit D-122. Mesure faite, ligne complétée par le commit suivant | — | — |
| D-127 | **RÉSERVÉ — `test_each_line_is_flushed_to_disk` : la durabilité du point de sauvegarde de H0 n'est gardée que par une recherche de chaîne.** Mutation A′ (`flush`+`fsync` déplacés sous une condition fausse, texte source intact) → **21 passed**, faux vert ; mutation B (réécriture ÉQUIVALENTE `_fd = fh.fileno()`) → **ROUGE**, faux rouge. Mesure faite, ligne complétée par le commit suivant | — | — |
