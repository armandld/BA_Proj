...
| D-152 | **Le garde anti-fuite d'identifiants ne balaie que `src/` — la fuite suivante n'a qu'a se poser ailleurs.** `test_no_hardcoded_credential_url_in_src` (`tests/pipeline/test_no_credential_in_source.py`), ecrit apres D-65 (URL Neon complete publiee dans un depot public). Reserve — mesures et correction poussees ensuite. | reserve | `pytest tests/pipeline/test_no_credential_in_source.py -q` |
