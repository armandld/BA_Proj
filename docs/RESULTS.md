...
| D-121 | Le repli de `sigma` sur 0,05 (`pipeline.py:394`) n'est vérifié que par un test qui lit le SOURCE — aucun test ne lit `result["sigma"]` | mutation : valeur réelle changée 0,05 → 0,07, tests rejoués | `pytest tests/pipeline/test_hyperparams_provenance_break.py tests/pipeline/test_train_hyperparams_smoke.py -q -k sigma` |
| D-122 | **RÉSERVÉ — `--zero-psi` sans `--with-psi` est une ablation VIDE, et un artefact publié porte ce nom.** Mesure en cours, ligne complétée par le commit suivant | — | — |
