...
| D-101 | **Réservé** — `fig5_qaoa_detailed_analysis.py` : `_gt_quadrant_above_threshold` compare le champ d'erreur GT brut (`ground_truth_errors`, non normalisé) à `threshold_amr`, un seuil calibré pour `classical_score` (normalisé au max du domaine, dans [0,1]) — les deux échelles sont incommensurables | mesure en cours | `pytest tests/study/test_fig5_gt_threshold_scale.py` |
