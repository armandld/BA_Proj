...
| D-161 | *(reserve — file des inventaires, 18 aout au soir)* **L'exemption de fuite d'identifiants ne peut pas perimer : les quatre couples exemptes figurent litteralement dans le source de l'inventaire, qui est lui-meme dans le corpus fouille.** `tests/pipeline/test_no_credential_in_source.py` | mesure en cours |
| D-162 | *(reserve — file des inventaires, 18 aout au soir)* **`test_every_entry_point_guards_its_main` ne verifie pas le bloc `__main__` dont il porte le nom, et `ENTRY_POINTS` est une categorie qui n'exige rien tout en dispensant des deux vrais controles.** `tests/pipeline/test_src_coverage_inventory.py` | mesure en cours |
| D-163 | *(reserve — file des inventaires, 18 aout au soir)* **Deux controles de peremption d'exemption verifient le FICHIER, pas la chose exemptee.** `tests/test_suite_integrity.py`, `tests/study/test_empty_sweep_never_silent.py` | mesure en cours |
| D-164 | *(reserve — file des inventaires, session concurrente du 18 aout)* **`COVERED` ne verifie pas la PROVENANCE de la couverture : un module peut passer pour teste par un homonyme d'attribut sans lien avec lui.** `tests/pipeline/test_src_coverage_inventory.py::test_each_covered_module_is_named_by_the_test_suite` | mesure en cours |
