...
locale : journal Optuna partagé, budget global, reprise contrôlée et
sorties atomiques. Les chemins HPC, bases distantes et anciens lanceurs V2
ont été retirés (`docs/CODE_REVIEW.md`).

**La campagne finale n'a pas encore été exécutée.** Il n'existe donc aucun
résultat numérique actuel autorisant une affirmation sur l'avantage
quantique — tout ce qui suit, dans le corps historique de ce document, est
un travail de falsification et de correction, pas la campagne
confirmatoire elle-même. Deux défauts ouverts touchent directement à ce
que produirait un lancement aujourd'hui, et devraient être tranchés avant
— voir `docs/DEFAUTS.md`, D-158 (l'agrégateur de la table maîtresse
plante sur le chemin canonique : personne ne peut aujourd'hui vérifier
qu'un nombre publié n'a pas dérivé) et D-191 (le bras QAOA est devenu
déterministe par défaut partout, pas seulement où le protocole
confirmatoire le documente vouloir).
...
python study/pipeline/dns_sweep.py --dry-run
```
...
every `run_arm` call site in the study tree's old `v4/` layout:
...
dans le vide (l'ancienne arborescence versionnée — `v3/` sous `study/` et
sous `tests/`, `study/results/`, `study/phase11_upper_bound.py`) — le
script datait de deux réorganisations en arrière. En prime `ROOT_DIR`
remontait de deux crans (`../..`) alors que le script est descendu dans
`scripts/`, un seul cran sous la racine : il désignait le **parent du
dépôt**. Sa seule étape qui « passait » était un `pytest` sur l'ancien
dossier `v3/` de `tests/`, vide : *no tests ran*, code de retour **0**.
...
Résultat de `scripts/inventaire_campagne.py` (retiré depuis — voir la note
en fin de section « Vérification » plus bas), commit `327d726` :
...
Historiquement, deux commandes retirées depuis (voir la note ci-dessous)
précédaient ce test : l'inventaire de campagne (script `inventaire_campagne`
sous `scripts/`, code 0 attendu, 26 bases) et le nettoyage des essais
fantômes (script `nettoyer_essais_fantomes` sous `scripts/`, options
`--toutes --dry-run`). Seul le test ci-dessous reste rejouable tel quel :

```bash
python -m pytest tests/pipeline/test_campagne_noms_et_fantomes.py -q
```

*(Note du 25 août : les deux scripts nommés dans le paragraphe qui précède
ce bloc — hors bloc de commande, puisqu'ils ne sont plus exécutables — ont
depuis été retirés du dépôt, dans le même mouvement qui a consolidé la
campagne sur une machine unique et un journal Optuna partagé avec reprise
contrôlée — `docs/CODE_REVIEW.md`, § « Décisions appliquées ». La mesure
des 298 essais fantômes ci-dessus reste un fait historique valide. Le test
ci-dessus, lui, existe toujours mais ne les importe plus — il couvre
aujourd'hui la validité des scripts de campagne, le modèle
une-machine-un-journal, et la reprise sur essai interrompu. Aucun
équivalent direct des deux gardes n'a été identifié ; à vérifier si le
mécanisme de reprise actuel les couvre.)*
...
|---|---|---|---|
| chemin | `t6_dynamic_gt.py` sous l'ancienne arborescence versionnée (`v3/`) | `study/pipeline/dynamic_patch_labels.py` | le dépôt a été réorganisé ; `phase2_hard_patches.py` est aujourd'hui `pipeline/hard_patch_labels.py` |
...
**Psi câblé, pas laissé à zéro.** Contrairement au jouet statique (une
seule paire de fonctions de flux, aucun instant précédent), cette
trajectoire en a un : `with_psi=True` avec l'avant-dernier instantané de
la même trace DNS comme `prev_fields` (mode documenté par
`prepare_qaoa_inputs` pour les études en paire d'instantanés, déjà
utilisé par `h0_optimiser_equivalence.py`). Trouvé et corrigé après coup
par `tests/study/test_psi_coverage_inventory.py` — la suite complète, pas
les sous-suites scopées utilisées jusque-là ; la première mesure
(psi=0 par omission) a été remesurée plutôt que publiée telle quelle. Les
deux tableaux ci-dessous sont donc post-correctif ; `exact_frac`/
`f1_classical` sont identiques à la première mesure (psi n'entre ni dans
l'optimum exact ni dans le seuil classique), seule la colonne QAOA change.
...
| 3 | mhd_rotor | 1,00 | 0,75 | 0,40 | 0,50 |
| 4 | mhd_rotor | 1,00 | 0,50 | 0,40 | 0,67 |
| 5 | double_tearing | 1,00 | 0,50 | 0,40 | 0,67 |
| 6 | lamb_oseen_vortex | 1,00 | 1,00 | 0,40 | 0,40 |
| 7 | island_coalescence | 1,00 | 1,00 | 0,40 | 0,40 |

**H0a répliquée** : QAOA ne s'effondre pas sur son propre optimum exact
(`agree_qaoa_exact` = 0,719 ± 0,232, min 0,5, max 1,0 — jamais moins de
50 % d'accord, jamais 100 % sauf aux trois graines où optimum exact ET
QAOA convergent tous deux sur qaoa_frac=exact_frac=1,0).

**H0b répliquée, et cette fois graine par graine.** F1(QAOA) = 0,629 ±
0,238 > F1(exact) = 0,425 ± 0,043 en moyenne, comme sur DNS réelle (T26)
et le jouet statique — mais ici l'inégalité tient AUSSI instance par
instance : `f1_qaoa >= f1_exact` aux 8 graines (égalité à 3/8, où les
deux arms convergent sur « tout raffiner » ; strictement supérieur à
5/8). Avec psi=0 (mesure précédente, remplacée), une graine (6) violait
l'inégalité (QAOA pire que l'exact) ; le câblage la corrige. L'échantillon
reste plus bruité que le jouet statique (n=8, plusieurs recettes
physiquement différentes, pas un pool i.i.d. d'une seule distribution
synthétique ; n=20 pour le jouet statique, une seule famille de bosses).
...
recettes d'instabilité randomisées, psi câblé) après l'avoir déjà fait
sur DNS réelle (T26) et le jouet statique, le même H0a et la même
direction H0b (QAOA bat l'optimum exact, en moyenne ET graine par graine
ici) — une réplication, pas une découverte isolée.

**N'établit PAS** : un écart quantique/classique — `f1_classical`
...
taille que N=256/dim=2 ; que l'inégalité graine par graine résisterait à
un échantillon plus grand — 8 graines suffisent à établir qu'elle N'EST
PAS automatique (elle a été violée avant le correctif psi), pas qu'elle
est garantie.

```bash
pytest tests/study/test_psi_coverage_inventory.py -v      # 6 passed
