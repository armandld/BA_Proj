...
| D-123 | **RÉSERVÉ — la garde de reprise de T22 : un `and` devient `or`, les quatre chaînes cherchées restent, 26/26 verts.** Mesure faite, ligne complétée par le commit suivant | — | — |
| D-124 | **RÉSERVÉ — `resumed_from_checkpoint` / `n_runs_resumed` : les deux écritures supprimées, les deux noms survivent DANS UN COMMENTAIRE, 26/26 verts.** Mesure faite, ligne complétée par le commit suivant | — | — |
| D-125 | **RÉSERVÉ — `h4_transfer_summary.py` cesse de filtrer les artefacts PARTIELS, la chaîne cherchée reste, 38/38 verts.** Mesure faite, ligne complétée par le commit suivant | — | — |
