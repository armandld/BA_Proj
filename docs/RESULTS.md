...
| D-146 | **Le garde qui doit tenir écrite la décision de ne pas corriger D-53 — le résultat le plus fort du dépôt — est satisfait par une simple référence croisée, et la décision a DÉJÀ quitté le fichier qu'il surveille sans qu'il le voie.** `test_the_decision_not_to_correct_stays_written` (`tests/study/test_h0_certified_dim3_contradicts_criterion.py`) fait `assert "D-53" in docs/DEFAUTS.md`. Réservé — mesures et correction poussées ensuite. | réservé | `pytest tests/study/test_h0_certified_dim3_contradicts_criterion.py -q` |
