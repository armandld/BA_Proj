...
| D-155 | **RÉSERVÉ — l'inventaire de la dette psi compte la PRÉSENCE du mot-clé, pas sa valeur : le seul script déclaré câblé peut passer `with_psi=False` sans qu'aucun test ne bouge.** Mesure et correction dans la même passe | en cours | `pytest tests/study/test_psi_coverage_inventory.py -q` |
