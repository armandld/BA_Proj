...
| D-149 | **Le « garde-fou du garde-fou » anti-D-31 compte une chaîne à l'espacement près : il reste vert si le bloc mort de `pipeline.py` devient du code, et rouge sur une simple réindentation.** `test_le_bloc_mort_ne_compte_pas` (`tests/pipeline/test_relative_percentile_is_trainable.py`). Réservé — mesures et correction poussées ensuite. | réservé | `pytest tests/pipeline/test_relative_percentile_is_trainable.py -q` |
