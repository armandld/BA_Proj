...
| D-160 | **RÉSERVÉ — la commande citée en prose (`` `python study/x.py --flag` ``) n'est vue par aucun garde d'existence, et l'exemption « historique » de `study/v4/` COMPTE les lignes au lieu de les identifier : remplacer une mention historique par une commande morte reste vert.** 10 commandes inline de `RESULTS.md` invisibles au balayage des chemins. Mesure et correction dans la même passe | en cours | `pytest tests/study/test_repro_commands_point_to_real_files.py -q` |
