...
| D-147 | **Même famille que D-146, sur le seul dossier du dépôt qui ne se régénère pas : `PROVENANCE.md` est gardé par des sous-chaînes.** `test_the_document_states_the_measured_numbers` (`tests/study/test_hyperparams_provenance.py`) cherche `"345"`, `"47"`, `"224"`… dans le document. Réservé — mesures et correction poussées ensuite. | réservé | `pytest tests/study/test_hyperparams_provenance.py -q` |
