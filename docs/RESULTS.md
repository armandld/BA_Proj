...
| D-109 | **RÉSERVÉ — le bloc `per_scenario` du JSON annonce l'optimum de chaque scénario et ne choisit que parmi les 3 meilleurs essais du score AGRÉGÉ.** Ligne posée avant écriture pour réserver le numéro (protocole `VIGIL.md`). Mesure, correction et test suivent dans le même fil. | en cours | en cours |
