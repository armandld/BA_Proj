...
| D-130 | **RÉSERVÉ — `--fixed-curl` du panel H0 : le garde qui vérifie que le drapeau ATTEINT le mappeur ne lit que le SOURCE.** Mutation A′ (le drapeau ne franchit plus `solver_panel`, les TROIS chaînes cherchées intactes) → **7 passed**, faux vert — et c'est mot pour mot ce que le message du test annonce empêcher (« il ne fait que renommer le fichier de sortie ») ; mutation B (réécriture ÉQUIVALENTE `bool(args.fixed_curl)`) → **ROUGE**. Mesure faite, ligne complétée par le commit suivant | — | — |
