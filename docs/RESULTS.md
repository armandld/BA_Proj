...
| D-108 | **RÉSERVÉ — `scripts/extract_best_hyperparams.py` laisse tomber le 9e paramètre de la campagne qu'il extrait, et ce paramètre perdu est tout ce qui sépare le JSON déployé de sa base d'origine.** Ligne posée avant écriture pour réserver le numéro (protocole `VIGIL.md`). Mesure, correction et test suivent dans le même fil. **À lire d'abord : cette entrée réfute une lecture publiée de `D-22`.** | en cours | en cours |
