...
| **D3** | `SCENARIOS_ALL` = ISOLATED + COMPLEX lists `ot` and `rotor` twice → 6 entries for 4 distinct classes | `train_hyperparams` | OT and rotor weighted 2:1 in every Phase-3 composite loss; for LOSO it would **manufacture leakage** (de-duplicated in the V4 driver). Also `SCENARIO_VORTEX` / `SCENARIO_COALESCENCE` defined but never used, contradicting the module's own docstring |
