...
Les **onze** à `scripts=1`, l'ordre de la file. **Six traités cette nuit, six
restent** — la file se compte, c'est ce que la règle d'arrêt demande d'elle :
...
| `study/h3_representation/h3_depth_report.py` | `--prune-eps` | **ouvert — non examiné**, mais il appelle la même `prune_hamilt_params` traitée deux lignes plus haut |
...
| `study/h2b_prediction/h2b_scenario_specialisation.py` | `--model` | **ouvert — non examiné.** Les deux côtés sont déjà *cités* plus haut (`--model lr` et GBT), mais aucun test ne les exécute |
...
| `study/h2b_prediction/h2b_multiseed.py` | `--n-seeds` | **ouvert — non examiné** |
| `study/h2b_prediction/h2b_variational_classifier.py` | `--reps-ansatz` | **lu, deux observations NON MESURÉES — à instruire, pas à croire.** (a) `run_vqc` enrobe `vqc.fit` d'un `warnings.catch_warnings()` + `simplefilter("ignore")` **inconditionnel** : c'est la forme exacte de D-38, où `Method Powell cannot handle constraints` s'est perdu sur stderr et la borne du mixeur avec lui. Ici le silence est **écrit dans le code**, pas subi. Si Qiskit signale que `maxiter` est ignoré, qu'un composant est déprécié ou que l'ajustement n'a pas convergé, personne ne le saura. (b) le commentaire annonce *« Scale to [-pi, pi] »* alors que le calcul rend **[−π/2, +π/2]** (`/span * π − π/2`) — famille D-175/D-179. La validation, elle, est clippée à **±π** : elle peut donc sortir de l'intervalle d'entraînement d'un facteur 2, ce qui est peut-être voulu et n'est écrit nulle part. **Aucune des deux n'est mesurée** : il faut exécuter le VQC, donc des artefacts. Une suspicion non mesurée n'est pas un défaut |
...
| `study/closed_loop/closed_loop_budget_matched.py` | `--max-iter` | **ouvert — non examiné** |
| `src/analyze_hyperparams.py` | `--full` | **ouvert — non examiné** |
