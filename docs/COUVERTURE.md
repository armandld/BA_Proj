...
| `src/pipeline.py` | `--eps`, `--grid-size`, `--in-file`, `--opt-level`, `--verbose` | **traité — module lu en entier (chemin critique, `main()`/`pipeline()`/`score()`), un drapeau mort trouvé, le reste vérifié vivant.** `--in-file` (« Input directory for mapping », défaut `../input/mapping_input.json`) est déclaré par `argparse` et **n'est jamais lu** : ni par `main()`, ni transmis à `pipeline()`, ni référencé ailleurs dans le dépôt (`grep -rn "in_file" src/ study/ scripts/ tests/` → une seule occurrence, la déclaration elle-même). Argument mort, forme D-48. Sans conséquence mesurable : aucune valeur ne dépend de lui, à aucun réglage, donc pas d'entrée `DEFAUTS.md` (règle d'arrêt). `--eps`, `--grid-size`, `--opt-level` : vérifiés vivants en suivant la chaîne complète `main→argus.eps/opt_level→run_adaptive_vqa→_run_level→call_vqa_shell→execute`/`VQARuntime.transpile` — `--opt-level` n'atteint `optimize()` (le point mort visible dans `call_vqa_shell`) que si `vqa_runtime is None`, ce qui n'arrive jamais depuis `pipeline()` (toujours construit sauf `classical_only`) : c'est `VQARuntime.__init__`/`.transpile` qui l'applique en pratique, chemin distinct mais vivant. Question 4 sur les deux sorties de score (`_details`/chemin de divergence contre `score()` final) : `instability_weight_map`/`weighted_relative_error` partagés, `patch_ratio` et `phys_score` recalculés identiquement aux deux points de sortie — coïncident, déjà réparé (commentaires en tête du fichier, D-9/D-22) |

Huit modules de plus « vérifiés et trouvés sains » pour éviter de les
relire (deux du 22 août, quatre de la passe précédente, deux de cette
passe). Rebalayé le 25 août (même script, même corpus) avant de retirer
`src/pipeline.py` : **70** drapeaux `scripts=0` restants pour les sept
modules déjà traités, pas 74 — écart de 4 avec le dernier chiffre écrit
ici, non expliqué, non chassé (mesuré, pas une inquiétude ; le module
suivant vérifiera si l'écart bouge encore). **65** restent après ce
module.
