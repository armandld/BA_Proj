...
sa valeur de REFERENCE publiee dans `study/v4/RESULTS.md` et d'un statut
...
Usage :
  python study/v4/aggregate_master_table.py
  python study/v4/aggregate_master_table.py --strict   # code retour != 0 si DIFF
"""
...
def to_markdown(rows, git_hash):
    lines = ["# V4 master table",
             "",
             f"Generated by `study/v4/aggregate_master_table.py` at commit "
             f"`{git_hash[:12]}`. Every row carries the reference value "
             "published in `study/v4/RESULTS.md`; MISSING marks a study "
