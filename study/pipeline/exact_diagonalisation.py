#!/usr/bin/env python3
"""Phase 4: exact solution of the diagonal Ising Hamiltonian.

The Z/ZZ/ZZZZ operator is enumerated in the computational basis without a
dense matrix. Every selected snapshot is compared to its L2-hard label and
saved for phase 5. ``promising`` remains a descriptive column only.
...
Usage:
  python study/pipeline/exact_diagonalisation.py --re 800 --dim 2
"""
...
def build_patch_hamiltonian(vx, vy, Bx, By, N, n_patches, Re,
    ...
    Returns the coefficient dictionary, the block-maximum classical score
    on the VQA grid, and the full-resolution classical score.
    """
    dx = 2 * np.pi / N
    nu = 1.0 / Re
    eta = 1.0 / Re
...
def exact_diag(hamilt_params, dim):
    """Enumerate the diagonal Ising Hamiltonian exactly.
    ...
    from ising_terms_and_annealing import (
        build_ising_terms,
        exact_ising_spectrum,
    )

    h_bias, edges, plaqs = build_ising_terms(hamilt_params, dim)
    spins, ground_energy, energies = exact_ising_spectrum(
        h_bias, edges, plaqs, n_qubits, max_qubits=20)
    basis_index = int(sum(
        (int(spin) == -1) << qubit for qubit, spin in enumerate(spins)))
    ground_state = np.zeros(1 << n_qubits, dtype=np.complex128)
    ground_state[basis_index] = 1.0
    gap = energies[1] - energies[0] if len(energies) > 1 else 0.0

    return ground_state, float(ground_energy), energies, float(gap)
...
def analyze_snapshot(vx, vy, Bx, By, N, n_patches, Re,
    ...
    ground_degeneracy = int(np.sum(np.isclose(
        energies, ground_energy, rtol=0.0, atol=1e-9)))
    ...
    # Descriptive comparison only; phase 5 evaluates the complete panel.
    ...
    return {
        ...
        "f1_tie": f1_tie,
        # A useful descriptive gain excludes constant decisions and F1 ties.
...
def run_phase4(dns_path, patches_path, n_patches, use_v2=False):
    ...
    degeneracies = [r["ground_degeneracy"] for r in all_results]
    ...
    print(f"    Unique ground state: "
          f"{sum(n == 1 for n in degeneracies)}/{len(degeneracies)}")
    ...
    if degenerate_snaps:
        print(f"    ... DEGENERATE: {len(degenerate_snaps)}/{len(all_results)}"
              ...
              f"cellule, le F1 ne mesure pas un accord spatial")
...
def artifact_name(scenario, Re, N, n_patches, suffix=""):
    """Return the phase-4 artifact name used by writers and readers."""
...
def save_results(all_results, meta, outdir=RESULTS_DIR, *,
    ...
    ground_degeneracy = np.array(
        [r["ground_degeneracy"] for r in all_results], dtype=np.int64)
    ...
    # These columns distinguish a real spatial gain from constant/tied masks.
...
def main():
        ...
        for re in args.re:
            for dim in args.dim:
                ...
                if results is not None:
                    save_results(
                        results, meta, run_provenance=run_provenance,
                        cli_args=cli_args)
    ...
    if not all_meta:
        raise RuntimeError(
            ...
            "au-dessus du plafond de 20 qubits")
