...
from qiskit.circuit.library import UnitaryGate, AndGate, PauliEvolutionGate, StatePreparation
...
def main():
    ...
    """
    Liste des paramètres thetas
    Vecteur c de l'ansatz
    ...
    # Number of qubits you want for total circuits : n
    n_qbits = args.numqbits

    # Number of qubits you want for Fourier coeffs : m
    m_qbits = args.Mqbits

    
    qc = QuantumCircuit(n_qbits)

    # C'est l'implémentation "efficace" (Shende-Bullock-Markov)
    prep_gate = StatePreparation(vector)

    # Tu peux donner un nom pour qu'elle soit lisible sur le dessin
    prep_gate.label = "My_ZGR_State"

    qc.append(prep_gate, m_qbits)
    qc.append(prep_gate, m_qbits)
